(*****************************************************************************
    Author: Kenneth M. Murry
            ...
            Project 2
	 Purpose:
	      lexan is responsible for extracting one lexeme from the lsit on each
...
get_program lexan =
	match lexan with
	  hd::tl -> begin
					match get_id hd with
					| -1 -> -1 (*If -1 an error has happened*)
					|  _  -> begin(*If the current position id is valid *)
					  			(* Check what the current position id is *)
								 match (List.hd lexan) with
								 | '{' -> begin
								 			  (*get_declaraction*)
								 			  Printf.printf ("Get <decl>");
								 			  0								 			  
								 		  end
								 | ';' -> begin
								 			  ...
								 			  (*get_assign*)
								 			  Printf.printf ("Get <assign");
								 			  0
								 	
								          end
								 | '}' -> begin
								 			  (*End of program*)
								 			  Printf.printf ("End of program found");
								 			  0
								          end
								 | _ -> begin
				    			 		    Printf.printf ("TEST!");
...
********************************************************)
get_declaraction lexan =
	match lexan with
	| hd::tl ->begin
					(*get_type <int> or <float>*)
					Printf.printf ("Get <type>");
					0
	           end
	| _ -> -1 (*If -1 an error has happened*)
end;;
